...
source_directories="$HOME/Desktop/audio-analysis/source_directories.json"
source_extensions="$HOME/Desktop/audio-analysis/source_extensions.json"
source_dates="$HOME/Desktop/audio-analysis/source_dates.json"
source_durations="$HOME/Desktop/audio-analysis/source_durations.json"
    ...
    find "$dir" -type f | while read -r file; do
        ...
        if [[ "$extension" =~ ^(wav|mp3|aiff|aif|m4a|flac|ogg|mov|mp4|flv|mkv)$ ]]; then
            ...
            if [ -e "$output_folder/$filename.json" ]; then
                ...
                meta_date=$(stat -f "%SB" -t "%Y-%m-%d" "$file")
                meta_duration=$(echo "$meta_stats" | awk -F'"' '/"duration":/ && !found {print int($4 + 0.5); found=1}' found=0)
                ...
                if [ -z "$spread_mean" ] || [ -z "$mean_volume" ]; then
                    ...
                    # Directoryname
                    echo '"'$file_counter'" : [ "'$directoryname'" ],' >> "$tmp_directories"

                    # Extensionname
                    echo '"'$file_counter'" : [ "'$extension'" ],' >> "$tmp_extensions"

                    # Date
                    echo '"'$file_counter'" : [ "'$meta_date'" ],' >> "$tmp_dates"

                    # Date
                    echo '"'$file_counter'" : [ "'$meta_duration'" ],' >> "$tmp_durations"
...
# Source dirs
remove_trailing_comma "$tmp_directories"
mv "$tmp_directories" "$source_directories"

# Source extensions
remove_trailing_comma "$tmp_extensions"
mv "$tmp_extensions" "$source_extensions"

# Source dates
remove_trailing_comma "$tmp_dates"
mv "$tmp_dates" "$source_dates"

# Source dates
remove_trailing_comma "$tmp_durations"
mv "$tmp_durations" "$source_durations"
